...
const log = require('../../logger').dbCredential;
...
dao.associateCredentialWithScopes = function (id, type, scopes) {
  ...
  if (!scopes) {
    log.debug(`associateCredentialWithScopes - ${Date.now() - t}ms`);
...
dao.dissociateCredentialFromScopes = function (id, type, scopes) {
  ...
  return Promise.all(dissociationPromises)
    .then((result) => {
      log.debug(`dissociateCredentialFromScopes - ${Date.now() - t}ms`);
      return result;
    })
    .catch((error) => {
      log.debug(`dissociateCredentialFromScopes - ${Date.now() - t}ms`);
...
dao.getAllCredentials = function (consumerId) {
  ...
  return Promise.all(awaitAllPromises)
    ...
    .then((result) => {
      log.debug(`getAllCredentials - ${Date.now() - t}ms`);
      return result;
    })
    .catch((error) => {
      log.debug(`getAllCredentials - ${Date.now() - t}ms`);
