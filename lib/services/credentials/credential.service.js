...
s.getCredentials = function (consumerId, options) {
  ...
  return credentialDao.getAllCredentials(consumerId)
    ...
    .then((result) => {
      log.debug(`getCredentials - ${Date.now() - t}`);
      return result;
    })
    .catch((error) => {
      log.debug(`getCredentials - ${Date.now() - t}`);
...
s.addScopesToCredential = function (id, type, scopes) {
  ...
  return Promise.all([
    ...
    .then((result) => {
      log.debug(`addScopesToCredential - ${Date.now() - t}`);
      return result;
    })
    .catch((error) => {
      log.debug(`addScopesToCredential - ${Date.now() - t}`);
...
s.removeScopesFromCredential = function (id, type, scopes) {
  ...
  return this.getCredential(id, type)
    ...
    .then((result) => {
      log.debug(`removeScopesFromCredential - ${Date.now() - t}`);
      return result;
    })
    .catch((error) => {
      log.debug(`removeScopesFromCredential - ${Date.now() - t}`);
...
s.setScopesForCredential = function (id, type, scopes) {
  ...
  return this.getCredential(id, type)
    ...
    .then((result) => {
      log.debug(`setScopesForCredential - ${Date.now() - t}`);
      return result;
    })
    .catch((error) => {
      log.debug(`setScopesForCredential - ${Date.now() - t}`);
