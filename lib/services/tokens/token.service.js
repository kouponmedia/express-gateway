...
const log = require('../../logger').servicesToken;
...
s.findOrSave = function (tokenObj, options = {}) {
  ...
  return this.find(tokenObj, options)
    ...
    .then((result) => {
      log.debug(`findOrSave - ${Date.now() - t}ms`);
      return result;
    })
    .catch((error) => {
      log.debug(`findOrSave - ${Date.now() - t}ms`);
...
s.getTokenObject = function (refreshToken) {
  ...
  return this.get(refreshToken, { type: 'refresh_token' })
    ...
    .then((result) => {
      log.debug(`getTokenObject - ${Date.now() - t}ms`);
      return result;
    })
    .catch((error) => {
      log.debug(`getTokenObject - ${Date.now() - t}ms`);
...
s.getTokensByConsumer = function (id, options) {
  ...
  return tokenDao.getTokensByConsumer(id, options)
    .then((result) => {
      log.debug(`getTokensByConsumer - ${Date.now() - t}ms`);
      return result;
    })
    .catch((error) => {
      log.debug(`getTokensByConsumer - ${Date.now() - t}ms`);
...
s.revoke = function (accessToken) {
  ...
  return this.get(accessToken).then(token => {
    ...
    .then((result) => {
      log.debug(`revoke - ${Date.now() - t}ms`);
      return result;
    })
    .catch((error) => {
      log.debug(`revoke - ${Date.now() - t}ms`);
...
s.createJWT = function (payload) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((result) => {
      log.debug(`createJWT - ${Date.now() - t}ms`);
      return result;
    })
    .catch((error) => {
      log.debug(`createJWT - ${Date.now() - t}ms`);
