...
function encrypt (text) {
  ...
  log.debug(`encrypt - ${Date.now() - t}ms`);
  return result;
}
...
function decrypt (password) {
  ...
  log.debug(`decrypt - ${Date.now() - t}ms`);
  return result;
}
...
function compareSaltAndHashed (password, hash) {
  ...
  log.debug(`compareSaltAndHashed - ${Date.now() - t}ms`);
  return result;
}
...
function saltAndHash (password) {
  ...
  if (!password || typeof password !== 'string') {
    log.debug(`saltAndHash - ${Date.now() - t}ms`);
    return Promise.reject(new Error('invalid arguments'));
  }

  const saltRounds = parseInt(process.env.SALT_ROUNDS);
  return bcrypt
    .genSalt(isNaN(saltRounds) ? config.systemConfig.crypto.saltRounds : saltRounds)
