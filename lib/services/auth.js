...
s.authenticateCredential = function (id, password, type) {
  ...
  const t = Date.now();
  if (!id || !password || !type) {
    log.debug(`authenticateCredential - ${Date.now() - t}ms`);
...
s.authenticateToken = function (token) {
  ...
  return tokens.get(token)
    ...
    .then((result) => {
      log.debug(`authenticateToken - ${Date.now() - t}ms`);
      return result;
    })
    .catch((error) => {
      log.debug(`authenticateToken - ${Date.now() - t}ms`);
...
s.authorizeToken = function (_token, authType, scopes) {
  ...
  if (!scopes || scopes.length === 0) {
    log.debug(`authorizeToken - ${Date.now() - t}ms`);
...
s.authorizeCredential = function (id, authType, scopes) {
  ...
  if (!scopes || !scopes.length) {
    log.debug(`authorizeCredential - ${Date.now() - t}ms`);
    return Promise.resolve(true);
  }
...
s.validateConsumer = function (id, options = {}) {
  ...
  return applications.get(id)
    ...
    .then((result) => {
      log.debug(`validateConsumer - ${Date.now() - t}ms`);
      return result;
    })
    .catch((error) => {
      log.debug(`validateConsumer - ${Date.now() - t}ms`);
